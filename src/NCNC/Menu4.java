...
public class Menu4 {
   ...
   public static void insertForSale() {
      ...
      try {
         ...
         while (true) {
            System.out.print("등록할 매물의 차량번호를 입력하세요 (ex : 00AA0000) : ");
            ...
            if (exist == true) {
               System.out.println("이미 존재하는 Vnum 입니다");
               exist = false;
            } else {
               if (orderVnum.trim().matches("^[0-9]{2}[A-Z]{2}[0-9]{4}$")) {
         ...
         System.out.println("등록할 차량의  정보를 입력하세요.");

         sb1.append("insert into vehicle values ('" + orderVnum + "'");
         ...
         sb1.append(", to_date('" + model_year + "', 'yyyy-mm-dd')");
         ...
         sb1.append(", " + String.valueOf(Stemp));
         ...
         while (rs.next()) {
            System.out.println(
                  "번호 : " + String.valueOf(rs.getInt("fuel_id")) + ", 종류 : " + rs.getString("fuel_type"));
         }
         ...
         sql = "select * from colors order by color_id";

         rs = stmt.executeQuery(sql);

         while (rs.next()) {
            System.out.println(
                  "번호 : " + String.valueOf(rs.getInt("color_id")) + ", 종류 : " + rs.getString("color_name"));
         }
         ...
         sql = "select * from engine_displacement order by ed_id";
         ...
         sql = "select * from transmission order by t_id";
         ...
         sql = "select * from maker order by maker_id";

         rs = stmt.executeQuery(sql);

         while (rs.next()) {
            System.out.println(
                  "번호 : " + String.valueOf(rs.getInt("maker_id")) + ", 종류 : " + rs.getString("maker_name"));
         }
         ...
         while (true) {
            ...
            if (!Stemp.equals("")) {
               ...
               System.out.println("값을 입력해주새요");
            }
         }

         int maker = Integer.parseInt(Stemp);
         ...
         sql = "select * from model where maker_no =" + String.valueOf(maker) + " order by model_id  ";

         rs = stmt.executeQuery(sql);

         while (rs.next()) {
            System.out.println(
                  "번호 : " + String.valueOf(rs.getInt("model_id")) + ", 종류 : " + rs.getString("model_name"));
         }
         ...
         int model = Integer.parseInt(Stemp);

         sql = "select min(detail_id), max(detail_id) from detailed_model where mno = " + String.valueOf(model)+ " order by detail_id  " ;
         ...
         while (rs.next()) {
            System.out.println(
                  "번호 : " + String.valueOf(rs.getInt("detail_id")) + ", 종류 : " + rs.getString("detail_name"));
         }
         ...
         sql = "insert into Order_info(Order_num, seller, Vnum) values (" + orderNum + ", '" + orderID + "', '"
               + orderVnum + "')";
      ...
      try {
         ...
         if (isSelect == true) {
            ...
            if (isSelect == true) {
               ...
               sql = "select min(detail_id), max(detail_id) from detailed_model where mno = "
               ...
               sql = "select * from detailed_model where mno =" + String.valueOf(model) + " order by detail_id ";

               rs = stmt.executeQuery(sql);

               while (rs.next()) {
                  System.out.println("번호 : " + String.valueOf(rs.getInt("detail_id")) + ", 종류 : "
      ...
      try {
         ...
         while (true) {
            ...
            if (Stemp.equals("Y") || Stemp.equals("y")) {
               ...
               sql = "select * from order_info where buyer is null order by order_num";
               ...
               while(rs.next()) {
                  ...
                  System.out.printf("매물번호 : %-5d 판매자 : %-10s 차량변호 : %-12s\n", order_num, seller, Vnum);
               }

               while (true) {

                  System.out.print("비공개 처리할 매물번호를 입력하세요! (원치않으면 공백을 입력하세요) : ");
                  Stemp = scan.nextLine().trim();
                  ...
                  if (Stemp.equals("")) {
                     ...
                     if (!Stemp.matches("^[0-9]*$")) {
                        ...
                        sql = "select * from order_info where buyer is null and Order_num = "
                              + String.valueOf(Stemp);
               ...
               sql = "select * from filter order by order_num";
               
               rs = stmt.executeQuery(sql);
               
               while(rs.next()) {
                  int order_num = rs.getInt("order_num");
                  System.out.printf("매물번호 : %-5d\n", order_num);
               }
