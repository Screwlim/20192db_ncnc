...
<%
   ...
   if (session.getAttribute("sessionID") == null) {
      response.sendRedirect("LoginPage.jsp"); // 로그인 안되있으면 로그인 페이지로 옮겨감 추후에 첫화면으로 바꿔야함!
   }
   try {
                  ...
                  <div style="margin-left: 10px">
                     <%
                        String sql = "select order_num, Vnum from order_info where buyer = \'"
                           ...
                           sql = "select order_num, Vnum from order_info where seller = \'"
                                 + session.getAttribute("sessionID") + "\' order by order_date desc";

                           rs = stmt.executeQuery(sql);
                           
                           if (rs.next()) {
                              out.print("<b>&nbsp판매 내역</b><br>");
                              out.print("<a href=\"#\" onclick=\"orderDetail(" + rs.getString("order_num") + ")\">" + i++ + ". "
                                    + rs.getString("Vnum") + "</a></br>");
                              while (rs.next()) {
                                 out.print("<a href=\"#\" onclick=\"orderDetail(" + rs.getString("order_num") + ")\">" + i++
               ...
               <div class="search_condition">
                  <form action="board.jsp" method="post" name="search_condition">
                     차종 : <select id="category" name="category">
                        ...
                        <option value=" ">Anything</option>
                        <option value="and c.tnum =0 ">Auto</option>
                        <option value="and c.tnum =1 ">Semi-Auto</option>
                        <option value="and c.tnum =2 ">Manual</option>
                     ...
                     <button class="refresh_button" >검색</button>
                  </form>
               </div>
