...
<%
	...
	try {
		...
		String sql = "SELECT order_num, buyer, extract(year from order_date) as order_year, extract(month from order_date) as order_month, extract(day from order_date) as order_day, seller, vnum, price, mileage, extract(year from model_year) as year FROM vehicle join order_info on vehicle_num = vnum WHERE order_num = " + idx;
		...
		if(rs.next()){
			...
			order_year = rs.getString("order_year");
			order_month = rs.getString("order_month");
			order_day = rs.getString("order_day");
		}
	...
	<div class="container">
		<form name="info" action="car_update.jsp" method="post">
			...
			<div class="row">
				...
				<div class="col-75">
					<input type="text" value="<%=buyer%>" id="buyer" name="buyer" placeholder="구매자" disabled>
				</div>
			</div>
			<div class="row">
				<div class="col-25">
					<label for="fname">구매 시각</label>
				</div>
				<div class="col-75">
					<input type="text" value="<%=order_year+"-" + order_month + "-" + order_day %>" id="order_date" name="order_date" placeholder="구매 시각" disabled>
				</div>
			</div>
			<div class="row">
				<div class="col-25">
